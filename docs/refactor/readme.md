...
**V1.7(多 horizon 期限结构预测 + 二阶/轨迹特征,见 [v1.7-progress.md](v1.7-progress.md)):** post-V1.6 的预测粒度 + 特征深度增强,不改骨架。**设计成稿,待实现。**

| 项 | 内容 | 状态 |
|---|---|---|
| **多 horizon 预测** | 每资产 × 固定网格 {5d,20d,60d} 各一条(砍 next_1d),唯一性约束改 asset×horizon;下游账本/结算/scorecard 已支持,主改 schema/prompt → 评估网格变密 | ☐ |
| **二阶/轨迹特征** | `factors.py` 加少量代码算的二阶信号(动量加速度 / 信号轨迹 / frog-in-the-pan 形状标量),给 LLM 建期限结构的原料;不喂原始日线;走 A/B 裁决 | ☐ |

**V1.8(新闻指标化,见 [v1.8-progress.md](v1.8-progress.md)):** 把新闻从"当天快照"升级成可量化时序指标,依赖前向积累,节奏独立。**设计成稿,待实现。**

| 项 | 内容 | 状态 |
|---|---|---|
| **新闻特征存储** | `news/store.py`:article-level parquet(月分区 + uuid 幂等 + extra/schema_version),两层存储(正文缓存 gitignored / 特征库入库,不含正文)| ☐ |
| **漏斗 + 逐篇分析** | 初判过滤(相关性/关键词)→ 仅幸存者喂逐篇分析 LLM 产 rich 标签写库;Basic `limit=25` 放开 | ☐ |
| **滚动情绪指标** | 日 → 5/20/60d 情绪走势 / 动量(二阶)/ 量 z + 情绪—价格背离;离线 lead-lag 研究(测不假设)| ☐ |
| **关键词 + linkage 事件层** | 每资产 + 宏观主题频道(Fed/财政/Trump/地缘/日韩)+ linkage_map 加"事件→资产机制";仅 B 臂、前向裁决 | ☐ |
