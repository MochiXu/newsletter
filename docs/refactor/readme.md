...
**V1.6(量化因子层 + 预测信心校准评估,见 [v1.6-progress.md](v1.6-progress.md)):** post-V1.5、pre-V2 的架构 + 方法论增强,不改骨架。**设计已成稿,待实现。**

| 项 | 内容 | 状态 |
|---|---|---|
| **量化因子层** | `factors.py`:原子特征 → 可解释因子打分(trend/momentum/value)+ 自产代码基线方向/信心 + EWMA 波动率预测;喂 LLM 当锚 + 落 `factors` 块 | ✅ |
| **信心语义钉死** | `confidence = P(方向成立)`(0~1)写进 schema/prompt,使其可校准 | ✅ |
| **评估层(前向)** | `evaluate.py`:vs-baseline 技能差 + 信心校准分桶 + Brier → `data/scorecard.json`;只跑**前向真实** `predictions.csv`(绕开回填记忆污染)| ✅ |
| **前端 S4** | 简报页「AI × 因子 × 基线」三方对照 + 波动率 chip + 信心 tooltip 校准提示;**命中率页点亮**(读 scorecard:A/B 消融 + 技能条 + 校准 + Brier,backfill 醒目重标)。月季年 heatmap/rollup 仍归 V2 | ✅ |
| **新闻信号层 + 多接口** | `news/` 多源可插拔(RSS + TheNewsAPI 3key/Cloudflare UA/源白名单;NewsAPI.org 留接口)+ trafilatura 抓全文;新闻→代码特征(量/净情绪/事件)进预测;**A/B 消融**(A 仅价格 / B 加新闻)+ `source` 诚实标注,评估层按臂裁决 | ✅(代码;全量刷新进行中) |

> **V1.7 + V1.8 一起做** —— 统一排期/依赖/里程碑见 [v1.7-v1.8-plan.md](v1.7-v1.8-plan.md)
> (核心:先抽共享 `tsfeatures` 工具箱、新闻语料尽早开始前向积累、v1.7 先见效)。

**V1.7(多 horizon 期限结构预测 + 二阶/轨迹特征,见 [v1.7-progress.md](v1.7-progress.md)):** post-V1.6 的预测粒度 + 特征深度增强,不改骨架。**后端已落地(177 测试绿);前端延后。**

| 项 | 内容 | 状态 |
|---|---|---|
| **多 horizon 预测** | 每资产 × {5d,20d,60d} 网格各一条(砍 next_1d);schema/prompt + render 三处(夹紧/共识/回填)改;下游账本/结算/scorecard 零改 | ✅ 后端 |
| **二阶/轨迹特征** | 共享 `tsfeatures.py` + `features.py` 加 mom_chg/trend_slope/vol_ratio/fip_60(经 snapshot 流入)+ prompt 渲染 | ✅ |
| **前端** | 假设层期限结构 + 命中率页网格变密 | ☐ 延后 |

**V1.8(新闻指标化,见 [v1.8-progress.md](v1.8-progress.md)):** 新闻升级成多族可量化时序指标。**后端已落地;前向积累 + 调度待运维。**

| 项 | 内容 | 状态 |
|---|---|---|
| **单一语料库 parquet** | `news/store.py` article-level(body + 标签;月分区 + uuid 幂等 + extra);Basic `limit=25`/per_asset 15;接入 pipeline 入库 | ✅ |
| **逐篇富标签 + textsignals** | LLM 加 sentiment 连续值;`textsignals.py` 代码算 EPU/GPR/鹰鸽/事件分类;`build_article_records` 入库 | ✅ |
| **八族特征** | 截面(情绪+分歧度/不确定/地缘/事件/category)✅ + 时序动态(`compute_news_trends`:MA/动量/量 z/背离)✅;新颖度/关系 ☐ 延后 | ◐ |
| **linkage 事件层 + 接入 + 裁决** | linkage_map 事件层 ✅ + prompt §9 接入(B 臂)✅;关键词升级 + 前向 scorecard 裁决 + lead-lag | ◐(裁决待积累)|
...
**73** 个离线单测通过、Rust 退役、CI 改 Python、前端按设计 3 页 SPA + 模型切换器/跨模型共识)。**V1.6 核心轮(S1 量化因子层 + S2 信心语义钉死 + S3 评估层)+ 新闻轮(S5:多接口 + TheNewsAPI + 正文抽取 + 新闻特征 + A/B 消融)均已落地实现并验证(124 测全绿,6 月全量回放重跑);前端轮 S4 待做。V2(完整预测价值评估)设计成稿待实现。**
