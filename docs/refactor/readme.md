...
| 项 | 内容 | 状态 |
|---|---|---|
| **量化因子层** | `factors.py`:原子特征 → 可解释因子打分(trend/momentum/value/carry/vol)+ 自产代码基线方向/信心 + 波动率预测;喂 LLM 当锚 + 落 `factors` 块 | ☐ |
| **信心语义钉死** | `confidence = P(方向成立)` 0–100,写进 schema/prompt,使其可校准 | ☐ |
...
**73** 个离线单测通过、Rust 退役、CI 改 Python、前端按设计 3 页 SPA + 模型切换器/跨模型共识)。**V1.6(量化因子层 + 前向信心校准)与 V2(完整预测价值评估)设计均已成稿,待实现。**
