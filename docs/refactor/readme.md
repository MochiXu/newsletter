...
>
> - V1 细节分步设计与进度 → [v1-progress.md](v1-progress.md)
> - V2 细节分步设计与进度 → [v2-progress.md](v2-progress.md)(报告的预测价值评估)
> - 本文件保留:动机 / 总架构 / 数据源分工 / 进度总表 / v2·v3 展望 / 技术栈与安全
> - 取代并汇总早期零散讨论稿 [`docs/data-analyze/`](../data-analyze/)(那两份是原始脑暴,不连贯)
...
3. **不偷看未来**——滚动特征只用过去窗口,因果安全;唯一风险是被修正的宏观数据(vintage),v1 用规避策略(见 v1-progress §3)。
...
| **V2-S4** | 前端真实历史接管(干掉 9 天 mock) | ☐ |
| **V2-S5** | 延伸多年(机制跑通后,统计意义 + 全周期) | ☐ |

**当前阶段:V1 已落地实现并验证**(纯 Python 管线端到端 DeepSeek 实跑、41 个离线单测通过、
Rust 退役、CI 改 Python)。V2 设计已成稿,待实现。
...
- **v3 · 多周期报告 + 深化**:在日报之上扩展 周/月/季/年报告(L2 周期特征层 + 各周期验证);多 prompt / 多模型质量评测集;若要把**宏观数据本身**放进被回测信号,这时才需 ALFRED vintage;历史新闻规模化(GDELT 多年)。
- **更远**:A 股/港股影响层、CFTC 持仓、FedWatch 隐含概率、Web dashboard(见 [docs/TODO.md](../TODO.md))。
