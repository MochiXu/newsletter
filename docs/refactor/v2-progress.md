...
S1 当初按单模型设计。v1.5 落地了多模型(每模型一份 `ModelView` + 代码级 `consensus`),几处要具体化。**本节只落设计,实现仍待 S2/S3。**

**(1) 预测账本升级到「逐预测结构化、分模型」**
- 现 `hypotheses.py` 的 `hypotheses.csv` 只存**主模型**的 `if_then/invalidation` 文本——没有结构化 `asset/direction/horizon`、不分模型 → 无法分模型打分、无法算共识命中。
- V2 账本改为逐预测行 `{date, model, asset, direction, horizon, confidence, key_factors, invalidation, actual{...}}`。共识不单独存(代码可由当日各模型预测重算)。
