            ...
            "items": {
                ...
                "properties": {
                    ...
                    "confidence": {
                        "type": "number",
                        "description": (
                            "**该预测方向在 horizon 内成立的主观概率**(0~1):0.5=没把握(等于抛硬币)、"
                            "0.7=七成把握。后续会按真实命中频率做**校准对账**(报 0.7 的那批是否真有七成命中),"
                            "所以**务必如实区分高/中/低把握**,别都给高、别都给同一个值。"
                        ),
                    },
                    "key_factors": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": (
                            "驱动该预测的关键因子(2~4 条),每条写成 '短标签|完整读数'(英文竖线分隔):"
                            "短标签是术语/指标名、尽量 8 字以内(如 'MA200 上方'、'VIX 中波'、'实际利率上行'),"
                            "完整读数带具体数值与口径(如 '纳指较 MA200 上方 12.3%'、'VIX 中等水平 16.78')。"
                            "都取自给定特征块,不要自行心算。"
                        ),
                    },
